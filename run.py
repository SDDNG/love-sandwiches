...
def validate_data(values):
    """
    Inside the try, coverts all strings into integers. Raises
    ValueError if strings cannot be converted into int, or 
...
def calculate_surplus_data(sales_row):
    """
    Compare sales with stock and calculate the surplus for each data type.
...
def get_stock_values(data):
    """
    Displays stock requirements for each sandwich type to the user
    """
    print("Make the following numbers of sandwiches for next market:.\n")
    headings = SHEET.worksheet("sales").row_values(1)

    stock_values = {headings[heading] : data[heading] for heading in range(len(headings))}
    print(stock_values)
    return    
